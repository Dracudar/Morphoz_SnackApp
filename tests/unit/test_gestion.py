...
def mocks_gestion(monkeypatch, id_cache):
    ...
    monkeypatch.setattr(
        "src.mod.saisie.backend.gestion_commande.restaurer_stock_plat",
        lambda plat: None,
    )
    monkeypatch.setattr(
        "src.mod.saisie.backend.gestion_commande.log_stock_restauration",
        lambda plat, id_cmd: None,
    )
    mock_cache = MagicMock()
    mock_cache.save = MagicMock()
    monkeypatch.setattr(
        "src.mod.saisie.backend.gestion_commande.get_stock_cache",
        lambda: mock_cache,
    )
    monkeypatch.setattr(
        "src.mod.saisie.backend.gestion_commande.print_ticket_recap",
        lambda *a: None,
    )
    monkeypatch.setattr(
        "src.mod.saisie.backend.gestion_commande.print_ticket_cuisine",
        lambda *a: None,
    )
    return mock_cache
