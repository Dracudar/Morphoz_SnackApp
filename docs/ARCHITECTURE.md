...
- `calculer_affluence(orders, date_from=None, date_to=None) → dict` — répartition horaire des **validations** de commande (`"Date de validation"`), filtrée sur cette même date. Une commande compte dès qu'elle a été validée quel que soit son statut final : une commande **annulée** a bien généré de l'affluence au moment de sa validation, même sans vente associée. Retourne :
  - `par_heure` : 24 tranches `"00h"`…`"23h"` agrégées sur toute la période.
  - `par_jour` : liste triée chronologiquement de `{date, par_heure, totaux}`, une entrée par date de validation présente dans la période (détail horaire jour par jour).
  - `par_semaine` : liste triée de `{semaine, quantite}` (libellé `"Sem. JJ/MM"`, date du lundi, sans année pour rester lisible sur l'axe) — nombre de commandes validées agrégé par semaine ISO.
  - `par_mois` : liste triée de `{mois, quantite}` (libellé `"MM/AAAA"`) — nombre de commandes validées agrégé par mois.
  - `moyenne_par_jour` : moyenne quotidienne de commandes validées sur la période (0 si aucun jour).
  - `totaux` : `nb_commandes_validees`, `nb_terminees`, `nb_annulees`, `nb_en_cours`.

  Deux constantes fixent, pour `UI.py` et `pdf_export.py`, la granularité affichée selon le nombre de jours distincts couverts par la période (`len(par_jour)`) — un événement court a besoin du détail horaire, mais celui-ci devient à la fois illisible et peu pertinent sur une période longue :
  - `SEUIL_JOURS_AFFLUENCE_DETAILLEE` (7) : en dessous ou égal, un graphique horaire par jour (`par_jour`) — adapté au suivi d'un événement jusqu'à la semaine.
  - `SEUIL_JOURS_AFFLUENCE_HEBDOMADAIRE` (31) : au-delà de `SEUIL_JOURS_AFFLUENCE_DETAILLEE` et en dessous ou égal à ce seuil, un graphique agrégé par semaine (`par_semaine`, environ un mois) ; au-delà, un graphique agrégé par mois (`par_mois`, jusqu'à l'année). Dans les deux cas, `moyenne_par_jour` est affichée en complément.
...
Génère le PDF (`reportlab.platypus`) : totaux, histogramme + tableau par plat, camembert + tableau par moyen de paiement, tableau des recettes pizza, histogramme du CA par jour, puis (si fournis) la section horaires d'affluence — un histogramme par date (`par_jour`) si la période couvre `SEUIL_JOURS_AFFLUENCE_DETAILLEE` jours ou moins, sinon la moyenne journalière suivie d'un histogramme agrégé par semaine (`par_semaine`) ou par mois (`par_mois`, au-delà de `SEUIL_JOURS_AFFLUENCE_HEBDOMADAIRE`) — puis histogramme + tableau du temps de préparation par plat, histogramme + tableau du délai de retrait par plat.
...
La section "Horaires d'affluence" affiche un graphique par date (`affluence["par_jour"]`) tant que la période filtrée couvre `SEUIL_JOURS_AFFLUENCE_DETAILLEE` jours ou moins (événement court, ex. ~48 h, jusqu'à une semaine) — au-delà, le détail horaire n'est plus lisible ni pertinent : la vue affiche la moyenne journalière (`affluence["moyenne_par_jour"]`) puis un graphique agrégé par semaine (`affluence["par_semaine"]`, jusqu'à `SEUIL_JOURS_AFFLUENCE_HEBDOMADAIRE`, ~un mois) ou par mois (`affluence["par_mois"]`, au-delà, jusqu'à l'année).

Les libellés des axes de catégories des graphiques `QChartView` (`_build_bar_chart`) sont verticaux (90°) plutôt qu'horizontaux : `QBarCategoryAxis` tronque les libellés en `"..."` dès que la largeur disponible par catégorie devient trop faible (calculée sur le nombre total de catégories, pas sur la longueur du texte voisin — incliner à 45° n'évite donc pas la troncature), alors que l'empreinte horizontale d'un libellé vertical reste minimale. `_build_bar_chart` fixe aussi une largeur minimale (~32px par catégorie, plafonnée à `_LARGEUR_FEUILLE`) sur le `QChartView`, sans quoi celui-ci se contente de son `sizeHint` naturel et la feuille parente ne s'étire pas jusqu'à sa largeur maximale même dans une fenêtre large.
